...
using GroupDocsAnnotationVisualStudioPlugin.GUI;
using GroupDocsAnnotationVisualStudioPlugin.Properties;

namespace GroupDocsAnnotationVisualStudioPlugin
{
    public class Connect : Extensibility.IDTExtensibility2, IDTCommandTarget
    {
        ...
        private const string GROUPDOCS_MENU_TOOLTIP = "Download and Run GroupDocs.Annotation for .NET API Examples";

        private const string Annotation_COMMAND_NAME = "GroupDocsAnnotationVSPlugin";
        private const string Annotation_COMMAND_CAPTION = "New GroupDocs.Annotation Example Project";
        private const string Annotation_COMMAND_TOOLTIP = "Download and Run GroupDocs.Annotation for .NET API Examples";
            ...
            try
            {
                ...
                for (int iloop = 1; iloop <= toolsPopup.CommandBar.Controls.Count; iloop++)
                {
                    if (toolsPopup.CommandBar.Controls[iloop].Caption == GROUPDOCS_MENU_CAPTION)
                    {
                        ...
                        {
                            if (cmdbtn.Caption == Annotation_COMMAND_CAPTION)
                            {
                                oBtn = cmdbtn;
                            }
                        }
                ...
                if (oBtn == null)
                {
                    ...
                    Command myCommand = commands.AddNamedCommand2(addInInstance, Annotation_COMMAND_NAME,
                        Annotation_COMMAND_CAPTION, Annotation_COMMAND_TOOLTIP, false, Resources.pnglogosmall, ref contextGUIDS,
           ...
           ref object varOut, ref bool handled)
        {
            try
            {
                ...
                {
                    if (cmdName == addInInstance.ProgID + "." + Annotation_COMMAND_NAME)
                    {
